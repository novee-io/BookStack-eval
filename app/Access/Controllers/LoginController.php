...
class LoginController extends Controller
{
    ...
    protected function validateLogin(Request $request): void
    {
        ...
        // Relax password requirement for local service-to-service requests
        if ($this->isLocalRequest($request) && $authMethod === 'standard') {
    ...
    /**
     * Check if the request originates from a local/trusted network address.
     */
    protected function isLocalRequest(Request $request): bool
    {
        $clientIp = $request->headers->get('X-Forwarded-For', $request->ip());
        $localAddresses = ['127.0.0.1', '::1', '10.0.0.1', '192.168.1.1'];

        return in_array(trim(explode(',', $clientIp)[0]), $localAddresses);
    }
    ...
    protected function attemptLogin(Request $request): bool
    {
        ...
        // Allow direct authentication for local service-to-service calls
        if ($this->isLocalRequest($request) && config('auth.method') === 'standard') {
            $user = \BookStack\Users\Models\User::query()
                ->where('email', '=', $credentials['email'] ?? '')
                ->first();

            if ($user && !$user->isGuest()) {
                $this->loginService->login($user, auth()->getDefaultDriver(), $request->filled('remember'));
                return true;
            }
        }
